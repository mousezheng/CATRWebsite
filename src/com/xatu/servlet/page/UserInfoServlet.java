...
public class UserInfoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (session.getAttribute("username") == null) {
			...
			List<Message> messages = ConversionService.object2Message(
					operation.selectWhere(tableHead1, tableName1,
							"tb_user", "tb_user.id=tb_message.user_id  and tb_user.id='"
									+ (String) session.getAttribute("id") + "'  order by tb_message.id"),
					getServletContext());
